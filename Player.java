...
public class Player extends Actor
{
    ...
    static boolean isInGrass, isInCave, isInWater;
    ...
    public void checkGrass()
    {
        if(isInGrass)
            ...
            {
                getWorld().addObject(new Transition("inWild", new WorldWildBattle("Forest")), 250, 200);
    ...
    public void checkCave()
    {
        switch(getWorld().getClass().getName())
        {
            case "WorldCaveStrange" : 
                if(Greenfoot.getRandomNumber(600) == 0 && isMoving)
                {
                    getWorld().addObject(new Transition("inWild", new WorldWildBattle("Cave")), 250, 200);
                    DataPlayer.checkWorld();
                }
                break;
        }
